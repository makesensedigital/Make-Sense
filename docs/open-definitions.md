...
|---|---|---|---|---|
| 1 | **Whether the trust seal stays.** A script injected from `trust-provider.com` with `document.write`, unpinned, without subresource integrity, running with full page privileges — and on plain HTTP resolving to `http://www.trustlogo.com/...`. §26 prohibits document-writing injection and requires third parties to enter through the tag container. | **Removed, 2026-08-12.** The hosting decision closed this rather than engineering preference: the seal advertises a **Sectigo certificate issued through DonWeb**, and from the cutover the site is served by GitHub Pages under a Let's Encrypt certificate. It would not have been merely a prohibited script, it would have been **a false claim about the site's own certificate**. There was no version of "keep it" left. If a trust signal is wanted later, a static image linking to the real certificate carries one without executing anything. | Juan Torresel | 2026-08-12 |
